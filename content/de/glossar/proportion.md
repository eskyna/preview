---
title: Proportion
term: Proportion
description:
  Proportion beschreibt das Verhältnis von Längen, Breiten und Volumen im Outfit und beeinflusst Balance,
  Silhouette und Blickführung.
category: Schnitt, Proportion & Passform
subcategory: Größen & Passform
tags:
  - proportion
  - schnitt
  - silhouette
  - passform
keywords:
  - proportion mode
  - proportionen outfit
  - schnitt
  - stilberatung
weight: 3
image: images/glossar/proportion.png
image_alt: Grafik mit Outfit-Linien und Flächen zur Erklärung von Proportion
relatedTerms:
  - title: Silhouette
    url: /glossar/silhouette/
  - title: Passform
    url: /glossar/passform/
  - title: Saumlänge
    url: /glossar/saumlange/
  - title: Layering
    url: /glossar/layering/
  - title: Taillierung
    url: /glossar/taillierung/
featured_glossar: true
featured_weight: 13
content_level: commercial-pillar
lastmod: "2026-07-09"
faq:
  items:
    - q: Kann Proportion wichtiger sein als Größe?
      a: Ja. Ein Teil kann die richtige Größe haben und trotzdem in der Länge oder Gewichtung unvorteilhaft wirken.
    - q: Brauche ich feste Figurregeln?
      a: Nein. Besser sind Vergleiche mit echten Outfits und deiner gewünschten Wirkung.
    - q: Wie erkenne ich gute Proportionen?
      a: Ein Outfit wirkt ruhiger, wenn Längen, Volumen und Blickpunkte bewusst zusammenspielen.
seoTitle: Proportion erklärt | ESKYNA Glossar
commercial_intent: true
commercial_weight: 21
commercial_cluster: fit
editorial_depth: mini-case-offer-bridge
consulting_questions:
  - Welche Linie unterstützt Proportion, Haltung und Beweglichkeit?
  - Welche Länge, Weite oder Platzierung macht den Unterschied?
  - Was lässt sich ändern und was sollte beim Kauf direkt stimmen?
consulting_focus:
  Proportion wird in der Beratung besonders relevant, wenn aus einem Begriff eine konkrete Entscheidung
  für Kleidung, Wirkung oder Garderobe werden soll.
offer_bridge:
  kicker: Passendes ESKYNA-Angebot
  title: Schnitt und Proportion praktisch prüfen
  text:
    Wenn ein Begriff mit Passform, Linie oder Länge zusammenhängt, entscheidet die Umsetzung am Körper. ESKYNA
    übersetzt Theorie in tragbare Silhouetten.
  bullets:
    - Proportionen klarer lesen
    - Schnitte gezielter auswählen
    - Änderungen und Neukäufe besser priorisieren
  primary_label: ESKYNA Stilgefühl ansehen
  primary_url: /stilgefuehl/
  secondary_label: Kleiderschrank-Check ansehen
  secondary_url: /garderobencheck/
---

## Definition

**Proportion**: Proportion beschreibt, wie die Teile eines Outfits zueinander stehen: Länge von Oberteil und Hose, Weite von Jacke und Rock, Höhe der Taille, Schuhform, Ausschnitt und Volumen. Gute Proportionen lassen ein Outfit ausgewogen wirken.

## Einfach erklärt

Proportion ist der Grund, warum ein ähnliches Outfit bei zwei Menschen ganz unterschiedlich aussehen kann. Ein langer Blazer kann strecken oder drücken, eine hohe Taille kann Beine verlängern oder den Oberkörper verkürzen, ein weiter Rock braucht vielleicht ein ruhigeres Oberteil. Dadurch verändert sich die [Silhouette](/glossar/silhouette/) sofort.

## Warum Proportion wichtig ist

Wenn Proportionen stimmen, wirkt Kleidung bewusster und hochwertiger. Du brauchst nicht unbedingt neue Trends, sondern oft nur andere Längen, andere Schuhe oder eine klarere Platzierung der Taille. Das verändert die [Silhouette](/glossar/silhouette/) sofort.

## Blick aus der Beratung

In der ESKYNA Beratung arbeite ich mit Proportionen sehr praktisch. Wir schauen, welche Längen deine Körperlinien aufnehmen, wo Blickpunkte entstehen und welche Balance zwischen Oberkörper, Unterkörper und Schulterlinie für dich stimmig ist.

## So nutzt du den Begriff praktisch

- Fotografiere Outfits frontal, um Längen besser zu sehen.
- Prüfe, wo Oberteile enden und wie das die Beinlinie verändert.
- Kombiniere weite Teile mit klaren Gegenpolen.
- Nutze Schuhe, Gürtel und Ausschnitt als Proportionswerkzeuge.

## Beispiel aus dem Alltag

Ein weiter Pullover über einem Midirock kann gemütlich wirken, aber die Beine optisch verkürzen. Wird der Pullover vorn leicht gesteckt, der Schuh heller gewählt oder die Jacke kürzer, entsteht sofort mehr [Balance](/glossar/balance/). Proportion zeigt, warum kleine Längenentscheidungen ein Outfit verändern.

Dasselbe gilt für Details wie [Ballonärmel](/glossar/ballonaermel/), die Volumen gezielt verschieben und deshalb ein ruhiges Gegengewicht brauchen.

## Wann Beratung sinnvoll wird

Beratung hilft, wenn du die richtige Größe kaufst und trotzdem das Gefühl hast, dass Outfits nicht sitzen. Oft liegt es nicht am Körper und nicht am Teil allein, sondern am Verhältnis der Längen und Volumen. Genau dort entsteht sichtbare Stimmigkeit.

## Typisches Missverständnis

Proportion bedeutet nicht, eine Figur zu korrigieren. Es geht nicht darum, einem Ideal zu entsprechen, sondern Kleidung so einzusetzen, dass sie deine Linien unterstützt und deine gewünschte Wirkung zeigt.

## Abgrenzung zu ähnlichen Begriffen

Proportion hängt eng mit [Passform](/glossar/passform/) zusammen, ist aber nicht identisch. Passform fragt, ob ein Teil am Körper sitzt. Proportion fragt, ob die Länge, Breite und Gewichtung im gesamten Outfit stimmen.

## Nächster Schritt

Wenn du den Begriff auf deine Garderobe übertragen möchtest, findest du einen passenden Einstieg bei den [ESKYNA Angeboten](/angebote/).

## ESKYNA-Merksatz

Proportion ist die Architektur eines Outfits.

<!-- commercial-depth-start -->

## Beratungssituation im echten Leben

Beim Thema **Proportion** wird die Frage in der Beratung konkret, sobald ein schöner Begriff in eine echte Entscheidung übersetzt werden muss: Was bleibt, was wird anders kombiniert, was einen Kauf wert ist und was du bewusst weglässt? Häufig zeigt sich, dass nicht ein einzelnes Kleidungsstück das Problem ist, sondern die fehlende Logik dahinter.

## Mini-Case: vom Zweifel zur Entscheidung

Typisch ist eine Person, die **Proportion** grundsätzlich versteht, aber vor dem Spiegel keinen sicheren Filter hat. Wir prüfen vorhandene Teile, vergleichen Alternativen und übersetzen den Begriff in zwei bis drei klare Regeln für Alltag, Beruf und besondere Anlässe. So wird aus Unsicherheit eine Entscheidung, die du wiederholen kannst.

## Entscheidungshilfe vor dem nächsten Kauf

- Passt die Entscheidung zu deiner gewünschten Wirkung?
- Unterstützt sie Farbe, Schnitt, Proportion und vorhandene Garderobe?
- Kannst du sie mit mindestens drei vorhandenen Teilen kombinieren?
- Fühlt sie sich auch nach einem langen Tag noch stimmig an?

## Woran du erkennst, dass du Unterstützung brauchst

Unterstützung ist sinnvoll, wenn du die Theorie kennst, aber im Laden, vor dem Spiegel oder beim Aussortieren zögerst. Dann hilft ein neutraler Blick, weil **Proportion** nicht isoliert betrachtet wird, sondern zusammen mit Körper, Rollen, Alltag, Budget und persönlicher Präsenz. Wenn du daraus eine klare Stilentscheidung machen möchtest, ist [ESKYNA Stilgefühl ansehen](/stilgefuehl/) der passende nächste Schritt.

<!-- commercial-depth-end -->

## Verwandte Begriffe

- [Silhouette](/glossar/silhouette/)
- [Passform](/glossar/passform/)
- [Saumlänge](/glossar/saumlange/)
- [Layering](/glossar/layering/)
- [Taillierung](/glossar/taillierung/)
